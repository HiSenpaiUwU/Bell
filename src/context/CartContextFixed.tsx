...
interface CartContextValue {
  ...
  clearCart: () => void;
  confirmOrder: (payment: PaymentMethod, checkoutDetails?: CheckoutDetails | null) => Order;
  clearLastOrder: () => void;
...
export function CartProvider({ children }: PropsWithChildren) {
  ...
  const { consumeAppliedCoupon, getDiscountedBreakdowns } = useRewards();
  ...
  useEffect(() => {
    ...
    setCartItems([]);
    setLastOrder(null);
  }, [canUseCart, setCartItems, setLastOrder]);

  const notifyLoginRequired = () => {
    notify({
      title: "Login required",
      message: "Please register or login before using the cart.",
      tone: "warning",
    });
  };
  ...
  return (
    <CartContext.Provider
      value={{
        cartItems: canUseCart ? cartItems : [],
        itemCount,
        lastOrder: canUseCart ? lastOrder : null,
        orderHistory,
        addProduct,
        addCustomizedMilkTea,
        updateQuantity,
